...
uploadsDirVar = server.mappath("passport_Files")
...
if procType="add" then
	...
	Rs.open "select * from passport where std_civil_id='"&std_civil_id&"'" , Conn,1,2
 	if Rs.eof then
		rs.addnew	
	...
	End if
if ArrImg(0)= "" then 	
		session("ErrMsg")="íÌÈ ÑÝÚ ÕæÑÉ ÇáÌæÇÒ "
		response.redirect "parentcp_passport_main.asp" 
Else
	rs.update
...
End if
	...
	conn.close
	set conn=nothing
	response.redirect "Parentcp_passport.asp" 
