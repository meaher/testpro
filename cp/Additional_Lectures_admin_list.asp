	...
	<tr>
		...
		<table border="0" width="98%" cellspacing="0" cellpadding="0" dir="rtl">
			<tr>
				...
				<div class="PageTitle">
					<p align="right">ÿ·» Õ’… «÷«›Ì…<br>
					<a href="Additional_Lectures_Admin_Add.asp">ÿ·» ÃœÌœ</a></div>
				
				<div class="RecordAdd">
				<form action="Additional_Lectures_admin_list.asp" method="post" >
				...
				%>
					<a target="_blank" href="Additional_Lectures_admin_Print.asp?ID=<%=id%>&smonth=<%=Request.form("SelectMonth")%>&sday=<%=Request.form("SelectDay")%>"><font size="3">ÿ»«⁄…</font></a></p>
				</form></div>
				...
				nRecRange = 15
				nRecPerRow = 1
				pageName="Additional_Lectures_Admin_list.asp?q=0"
				...
				if ID > 0 then
					if SelectDay > 0 then
						if SelectMonth > 0 then
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Department_id = "& id &" and First_Lecture_Day= "& SelectDay &" and First_Lecture_Month="& SelectMonth &" Order By status='open' "							
						else 
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Department_id = "& id &" and First_Lecture_Day= "& SelectDay &" and First_Lecture_Month="& month(Date()) &" Order By status='open' "							
						end if
					Else		
						if SelectMonth > 0 then
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Department_id = "& id &" and First_Lecture_Month="& SelectMonth &" Order By status='open' "							
						else 
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Department_id = "& id &" Order By status='open' "							
						end if
					End if
				Else
					if SelectDay > 0 then
						if SelectMonth > 0 then
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Lecture_Day= "& SelectDay &" and First_Lecture_Month="& SelectMonth &" Order By status='open' "							
						else 
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Lecture_Day= "& SelectDay &" and First_Lecture_Month="& month(Date()) &" Order By status='open' "							
						end if
					Else		
						if SelectMonth > 0 then
							sSql = "select * from Switch_Lectures where Additional= 1 and  First_Lecture_Month="& SelectMonth &" Order By status='open' "							
						else 
							sSql = "select * from Switch_Lectures where Additional= 1 Order By status='open' "							
						end if
					End if
	 			End if
						...
						<div>
							<table cellspacing="0" cellpadding="3" class="tblView">
								...
								<% End If %>
									...
									<%if rs("Status")="open" then%>
										<a href="Additional_Lectures_Admin_update.asp?Switch_lectures_ID=<%=rs("Switch_lectures_ID")%>">
										«·„Ê«›ﬁ…</a>
									<%End if%>
									</td>
									...
									<%if rs("Status")="open" then%>
										<a href="Additional_Lectures_Admin_Delete.asp?Switch_lectures_ID=<%=rs("Switch_lectures_ID")%>">
										Õ–›</a>
									<%End if%>
									</td>
