...
if procType="add" then
					...
					Second_Lecture_ID=Rss("Lecture_ID")
						Rsv.open "select * from Switch_Lectures where  First_Department_ID="&First_Department_ID&" and First_Employee_ID="&First_Employee_ID&" and 	First_Class_room_id="&First_Class_room_id&" and First_Lecture_Time_id="&First_Lecture_Time_id&" and First_Lecture_Day="&First_Lecture_Day&" and First_Lecture_Month="&First_Lecture_Month&" and First_Lecture_Year="&First_Lecture_Year&" and First_subject_id="&First_subject_id&"" , Conn,1,2
							...
							Else
								...
								response.redirect "Additional_Lectures_add.asp?id="&Upload.Form("First_Department_ID")&"&start=1&DeptName="&Upload.Form("txtDepartment_Name")&"" 
							End if
						rsv.close
						set rsv=nothing	
	...
	conn.close
	set conn=nothing
	response.redirect "Additional_Lectures_list.asp?id="&Upload.Form("First_Department_ID")&"&start=1&DeptName="&Upload.Form("txtDepartment_Name")&"" 
...
elseif procType="edit" then
		...
		if not rsv.eof then
			if status = "ok" then
				...
				Rs.open "select * from Lectures" , Conn,1,2
					if not rs.eof then
							Rss.open "select * from Lectures where Department_id="& rsv("First_Department_id")&" and Subject_ID="&rsv("First_Subject_id")&" and Employee_ID="&rsv("First_Employee_ID")&" and Class_room_ID="&rsv("First_Class_room_ID")&" and Lecture_Time_id="&Rsv("Second_Lecture_time_ID")&" and Lecture_Day="&rsv("Second_Lecture_Day")&" and Lecture_Month="&rsv("Second_Lecture_Month")&" and Lecture_year="&rsv("Second_Lecture_year")&"" , Conn,1,2
							if rss.eof then
								rs.addnew
								rs("Department_id")= rsv("First_Department_id")
								rs("Subject_ID")=rsv("First_Subject_id")
								rs("Employee_ID")=rsv("First_Employee_ID") 
								rs("Class_room_ID")=rsv("First_Class_room_ID") 
								...
								conn.execute "delete from Lectures where Lecture_ID="&Second_Lecture_ID&" "
							Else
	...
	conn.close
	set conn=nothing
	response.redirect "Additional_Lectures_list.asp?id="&Upload.Form("Second_Department_ID")&"&start=1&DeptName="&Upload.Form("txtDepartment_Name")&"" 
