...
if procType="add" then
	set Rs=server.createobject("ADODB.Recordset")
		...
		RsChk.open "select * from Work_Day where Work_Day="&Day(Date())&" and Work_Month="&Month(Date())&" and work_year="&Year(Date())&" and Employee_ID= "&Upload.Form("txtEmployee_ID")&"" , Conn,1,2
	if RsChk.eof then
		...
		rs("Work_Day") = Day(Upload.Form("hdnwork_date"))
		rs("Work_Month") = Month(Upload.Form("hdnwork_date"))
		rs("Work_Year") = Year(Upload.Form("hdnwork_date"))
...
set rsChk=nothing	
	...
	conn.close
	set conn=nothing
	response.redirect "Department_Add.asp?start=1&ID="&Upload.Form("txtHdnDptID")&"&DeptName="&Upload.Form("txthdnDptName")&"" 
