	...
	<tr>
		...
		<table border="0" width="98%" cellspacing="0" cellpadding="0" dir="rtl">
			<tr>
				...
				<div class="PageTitle">
					<p align="right"><span lang="ar-kw">„ «»⁄… «·ÿ·»… «·„ „Ì“Ì‰</span></div>


				<form action="Student_Follow_p_Department_list.asp?id=<%=Request.queryString("id")%>&DeptName=<%=Request.queryString("DeptName")%>&start=1" method="post" >
					...
					<input type="submit" value="≈—”«·" name="B1">&nbsp;&nbsp;&nbsp; 	<a target="_blank" href="Student_Follow_p_Department_list_Print.asp?id=<%=Request.queryString("id")%>&DeptName=<%=Request.queryString("DeptName")%>&EID=<%=Request.Form("SelectEmployeeID")%>">
				...
				nRecRange = 15
				nRecPerRow = 1
				pageName="Student_Follow_P_Department_list.asp?q=0&id="&id=request.querystring("id")
						...
						<div>
							<table cellspacing="0" cellpadding="3" class="tblView" width="955">
								<thead>
									...
									<td class="tdViewHead" width="41%" align="center">
								...
								<%if rs("style")= "yes" then%> ‰⁄„<%end if%></td>
								 ...
								 <%if rs("Care_Study_Tools")= "yes" then%> ‰⁄„<%end if%></td>
									...
									<td class="tdView" align="center" width="3%">
											<a href="student_Follow_P_Department_Edit.asp?id=<%=rs("ID")%>&Deptid=<%=request.querystring("id")%>&Dept_name=<%=request.querystring("deptname")%>&Class_Room=<%=Class_Room%>&std_name=<%=std_name%>">
											«· ›«’Ì· </a>
								...
								<% If (nRecActual Mod nRecPerRow = 0) Or (nRecPerRow < 2) Then %>
									...
									<td class="tdView" align="center" width="3%">
											<a href="student_Follow_P_Department_View.asp?id=<%=rs("ID")%>&Deptid=<%=request.querystring("id")%>&Dept_name=<%=request.querystring("deptname")%>&Class_Room=<%=Class_Room%>&std_name=<%=std_name%>">
											ÿ»«⁄…</a></td>
								</tr>
								<% 
									End If 
