	...
	<tr>
		...
		<table border="0" width="98%" cellspacing="0" cellpadding="0" dir="rtl">
			<tr>
				...
				<div class="PageTitle">
					<p align=right><span lang="ar-kw"></span>’ÕÌ›… ”·ÊﬂÌ… ÃœÌœ…</div>
				...
				ssql="select * from student_behavior where managment_done='no' order by Department_ID,Std_civil_id,Report_date desc"
								...
								<% End If %>
									...
									<td class="tdView" width="9%">&nbsp;
										...
										if rs("Department_ID")<>0 then
													set Rst=conn.execute("Select * from Departments where Department_ID="&rs("Department_ID")&"")
													if not rst.eof then
												%>
														<%=Rst("Department_Name")%>
								...
								<%if rs("wrong_behavior")= "yes" then%> ‰⁄„<%end if%></td>
								 ...
								 <%if rs("refusal_instructions")= "yes" then%> ‰⁄„<%end if%></td>
									...
									<td class="tdView" align="center" width="8%">
											<a href="student_behavior_Admin_update.asp?id=<%=rs("ID")%>&Class_Room=<%=Class_Room%>&std_name=<%=std_name%>&Page=1">
											«· ›«’Ì· Ê«·≈Ã—«¡</a>
