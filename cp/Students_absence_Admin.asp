	...
	<tr>
		...
		<table border="0" width="98%" cellspacing="0" cellpadding="0" dir="rtl">
			<tr>
				...
				<div class="PageTitle">
					<p><span lang="en-us">„ «»⁄… «·€Ì«» «·ÌÊ„Ì</span></div>
				
				<div class="RecordAdd">
				
<form action="Students_absence_Admin.asp?start=1" method="post" >
				...
				<!--		<a target="_blank" href="Students_absence_Admin_Print.asp?ID=<%=request.querystring("ID")%>&DeptName=<%=Request.querystring("DeptName")%>&sday=<%=Request.form("SelectDay")%>&smonth=<%=Request.form("SelectMonth")%>&EmployeeID=<%=Request.Form("SelectEmployee")%>">
					...
					-->
					<a target="_blank" href="Students_absence_admin_Print.asp?smonth=<%=Request.form("SelectMonth")%>&sDay=<%=Request.form("SelectDay")%>&SClass=<%=Request.form("SelectClass")%>"><font size="3">ÿ»«⁄…</font></a></p>
				</form>					
				</div>
				...
				nRecRange = 15
				nRecPerRow = 1
				pageName="Students_absence_Admin.asp?q=0"
	 		...
	 		End if
				...
				<% If nTotalRecs > 0 Then %>
						...
						<%if session("ADMIN_LEVEL")=4 or session("ADMIN_LEVEL")=1 or session("ADMIN_LEVEL")=3 then%>
															...
															<a href="Students_absence_Admin_del_chk.asp?id=<%=rs("Absence_ID")%>&Class_id=<%=rs("Class_room_id")%>&subject_id=<%=rs("subject_ID")%>&start=1">
																	Õ–› „‰ «·€Ì«»</a>
