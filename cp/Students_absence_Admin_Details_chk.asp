...
elseif Request.querystring("ID") <> "" then	
	...
	conn.close
	set conn=nothing
	response.redirect "Students_absence_Admin_Details.asp?Class_Room_id="&Request.queryString("Class_Room_ID")&"&Class_room="&Request.queryString("Class_room")&"&start=1"	
elseif Request.querystring("Excuse_ID") <> "" then	
	ID = Request.querystring("Excuse_ID")

	set Rs=server.createobject("ADODB.Recordset")
	Rs.open "select * from Students_absence where absence_ID="&ID&" " , Conn,1,2
		 
		if rs("Excuse") = "" or rs("Excuse")="0" then
			rs("Excuse")=1
		Else
			rs("Excuse")=0
		End if
		rs("Admin_id")=session("ADMIN_ID")
