					...
					<form action="Students_data_chk.asp" method="post" name="frmData" id="frmData" onsubmit="return ValidateData()"  enctype="multipart/form-data">
						...
						<table class="tblDataControl" cellpadding="3" cellspacing="0" dir="rtl">
							...
							<tr>
								...
								<td class="tdDataControlR"><input  type="text" value=<%=Std_Number%> name="txtStd_Number" class="textBox" onchange="ValidateData()"></td>
							</tr>
							<tr>
								<td class="tdDataControlL" align="left">«”„ «·ÿ«·»</td>
								<td class="tdDataControlR">
								<input  type="text" name="txtStd_Name" value="<%=Std_Name%>" class="textBox" onchange="ValidateData()"></td>
							</tr>
							<tr>
								<td class="tdDataControlL" align="left">«·—ﬁ„ «·„œ‰Ì</td>
								<td class="tdDataControlR">
								<input type="text" name="txtStd_Civil_ID" value=<%=Std_Civil_ID%> class="textBox" onchange="ValidateData()"></td>
							</tr>
							<tr>
								<td class="tdDataControlL" align="left">«·’›</td>
								<td class="tdDataControlR">
							<select size="1" name="SClass_Room_id" class="select">
						
										<%									
										set Rssd=conn.execute("Select * from Class_Room")
										while not rssd.eof
										%>
														<option value="<%=rssd("Class_Room_id")%>"  <%if Class_Room_ID=rssd("Class_Room_ID") then%> selected<%End if%>><%=Rssd("Class_Room")%></option>
										<%
											rssd.movenext
										wend
										set rssd=nothing
