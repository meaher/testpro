		...
		<table border="0" width="98%" cellspacing="0" cellpadding="0" dir="rtl">
			<tr>
				...
				<div class="PageTitle">
					...
					<a href="Switch_Lectures_Add.asp?id=<%=request.querystring("id")%>">ÿ·» ÃœÌœ</a></div>
				...
				id = request.querystring("id")
				sSql = "select * from Switch_Lectures where Additional= 0 and (First_Department_ID="&id&" or Second_Department_ID="&id&")"
							...
							<table cellspacing="0" cellpadding="3" class="tblView">
								...
								<% End If %>
									...
									<%if cint(id)=rs("Second_Department_id") and rs("Status")="open" then%>
										<a href="Switch_Lectures_update.asp?Switch_lectures_ID=<%=rs("Switch_lectures_ID")%>&id=<%=id%>">
										«·„Ê«›ﬁ…</a>
									<%End if%>
									</td>
									<td class="tdView" width="8%">
									...
									%>
									<a href="Switch_Lectures_Print.asp?Switch_lectures_ID=<%=rs("Switch_lectures_ID")%>&id=<%=id%>&DeptName=<%=request.queryString("DeptName")%>">
										ÿ»«⁄…</a>
									<%
									End if
									%>
									...
									<%if cint(id) = rs("First_Department_id") and rs("Status")="open" then%>
										<a href="Switch_Lectures_Delete.asp?Switch_lectures_ID=<%=rs("Switch_lectures_ID")%>&id=<%=id%>">
										Õ–›</a>
									<%End if%>
									</td>
