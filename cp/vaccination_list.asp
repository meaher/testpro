...
<table class=" w3-left w3-hide-large w3-hover-cyan w3-large w3-theme" border="0" width="100%" id="table1"  style="border-style: solid; background-color: #FFFFFF !important">
	...
	<table border="0" width="98%" cellspacing="0" cellpadding="0" dir="rtl">
			<tr>
				...
				<form action="MeetingAdmin_list.asp?start=1" method="post" >
					...
					<select size="1" name="SelectClass_room" class="select">
						<option Value="0">Ã„Ì⁄ «·’›Ê›</option>
										<%									
										set Rssd=conn.execute("Select * from class_room order by Class_Room_id asc")
										while not rssd.eof
										%>
				...
				nRecRange = 15
				nRecPerRow = 1
				pageName="vaccination_list.asp?q=0"
				...
				if ID > 0 then		
					sSql = "select * from Vaccination where Class_Room_id = "& id &" order by Reg_Date"
				Else
					sSql = "select * from Vaccination order by Class_Room_ID, Reg_date"
	 			End if
						...
						<div>
							<table cellspacing="0" cellpadding="3" class="tblView">
								...
								<% End If %>
									...
									<%
									if rs("Vaccination")=0 then
										response.write("·„ Ìÿ⁄„")
									elseif rs("Vaccination")=1 then
										response.write("Ã—⁄… Ê«Õœ…")
									elseif rs("Vaccination")=2 then
										response.write("Ã—⁄ Ì‰")
									elseif rs("Vaccination")=3 then
										response.write("„⁄›Ì ")
									End if
									%>
									...
									if rs("Vaccination")>0 then
									%>
										<a href="/Vaccination_Files/<%=rs("Vaccination_File")%>"> Õ„Ì·</a>
									<%End if%>
									</td>
